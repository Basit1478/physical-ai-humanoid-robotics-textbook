"""
Final script to properly ingest documents using Google's embedding API to generate 1536-dimensional vectors
"""
import asyncio
import os
import hashlib
...
def generate_google_embeddings(chunks: List[Dict]) -> List[Dict]:
    """
    Generate proper Google embeddings for the chunks that are semantically meaningful.
    """
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        # Get all texts for this batch
        texts = [chunk['text'] for chunk in batch]

        try:
            # Use Google's embedding API
            result = genai.embed_content(
                model='models/text-embedding-004',  # This model produces 1536-dimensional embeddings
                content=texts,
            ...
            for j, chunk in enumerate(batch):
                if j < len(embeddings):
                    ...
                    logger.error(f"No embedding returned for chunk {i+j}")

        except Exception as e:
            logger.error(f"Error generating embeddings for batch {i//batch_size + 1}: {str(e)}")
            # Fallback: use the previous approach for this batch
            for chunk in batch:
                chunk_copy = chunk.copy()
                # Use the improved embedding approach as fallback
                embedding = create_meaningful_embedding(chunk['text'], 1536)
...
def create_meaningful_embedding(text: str, dimensions: int = 1536) -> List[float]:
    ...
    This is used as a fallback when the Google API is not available.
    """
    import hashlib
...
async def main():
    """
    Main function to ingest local documents into Qdrant using Google embeddings.
    """
    logger.info("Starting ingestion with Google embeddings (1536-dimensional)...")
    ...
    try:
        ...
        # Generate proper Google embeddings (1536-dimensional)
        logger.info("Generating Google 1536-dimensional embeddings...")
        chunks_with_embeddings = generate_google_embeddings(chunks)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("GOOGLE EMBEDDINGS DOCUMENT INGESTION SUMMARY")
    print("="*60)
